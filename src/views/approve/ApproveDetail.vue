    ...
    <div class="box">
        ...
        <div class="title-out">
            ...
            <div class="title-other">
                <el-button @click="drawer = true" class="el-button-padding0"
                    style="height: 35px; font-size: 14px;width: 100px;">审批历史</el-button>
                <el-button class="el-button-padding0" @click="drawerAttach = true"
                    style="height: 35px; font-size: 14px;width: 60px;">附件</el-button>
            </div>
        </div>
        <!-- 基本信息 -->
        ...
        <!--    <el-form :inline="true" :disabled="true" class="demo-form-inline form-dl" label-width="130px">-->
        <!--      <el-form-item :label="item.name" v-for="(item,index) in header.cols" v-bind:key="index" :data-title="item.name">-->
        <!--          <div class="label-tips">{{item.name}}</div>-->
            ...
            <el-tabs class="tabs-dl" type="card">
                <el-tab-pane :label="body.tabname" v-for="(body,bindex) in bodyArray" :key="bindex">
                    <el-table :data="body.data" stripe style="width: 100%" class="table-dl">
                        <el-table-column :prop="bitem.code" :label="bitem.name" width="180"
                            v-for="(bitem,bodyIndex) in body.cols" :key="bodyIndex"></el-table-column>
                    </el-table>
                </el-tab-pane>
            </el-tabs>
        </div>
        ...
        <div class="dl-button-out">
            <div>
                <el-button class="dl-button dl-button-1" type="primary" @click="openMessage('agree','同意')"
                    v-if="status == '0'" round>同意</el-button>
                <el-button class="dl-button dl-button-1" type="primary" @click="openMessage('disagree','不同意')"
                    v-if="status == '0'" round>不同意</el-button>
                <el-button class="dl-button dl-button-1" type="primary" @click="cancleApprove()" v-if="status == '1'"
                    round>取消审批</el-button>
                <el-dropdown class="dl-button" placement="top-end" @command="doTask" v-if="status == '0'">
                    ...
                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item style="width:80px;text-align:center" v-for="(task,index) in taskInfo"
                            :key="index" :command="task">{{task}}</el-dropdown-item>
                    </el-dropdown-menu>
                </el-dropdown>
            </div>
        </div>
        <!-- 抽屉页面 -->
        <el-drawer title="审批历史" :visible.sync="drawer" class="approve" :with-header="false" :direction="direction">
            <h3>审批历史</h3>
            <el-timeline :reverse="reverse">
                <el-timeline-item :timestamp="history.dealdate" placement="top"
                    v-for="(history,index) in approveHistory" :key="index">
        ...
        <el-drawer title="附件" :visible.sync="drawerAttach" :direction="directionAttach" :with-header="false"
            class="attach-drawer">
        ...
        <!-- 弹窗 -->
        <el-dialog title="请选择加签对象" :visible.sync="dialogVisibleAdd" class="dialog-dl" width="60%" height="120%"
            top="10vh">
            <select-people :multi="true" :msgId="msgid" :userId="userId" @close="closeDialog" @submitForm="addSubmit">
            </select-people>
        </el-dialog>
        <el-dialog title="请选择改派对象" :visible.sync="dialogVisibleAssigned" class="dialog-dl" width="60%" height="120%"
            top="10vh">
            <select-people :multi="false" :msgId="msgid" :userId="userId" @close="closeDialog"
                @submitForm="assignedSubmit"></select-people>
        </el-dialog>
        <el-dialog title="请选择驳回节点" :visible.sync="dialogVisibleReject" class="dialog-dl" width="60%" height="120%"
            top="10vh">
    ...
    export default {
        ...
        methods: {
            ...
            getData: function () {
                let that = this;
                that
                    .post("/app/approvedRest/msgDetail", {
                        msgid: that.msgid
                    })
                    .then(data => {
                        this.getApproveDo();
                        let response = data.res.responcontext;
                        if (response) {
            ...
            dealData: function (element, billInfo) {
                ...
                const cols = element["cols"].sort(this.compare('showOrder'));
                if (data) {
        ...
        compare(property) {
            ...
            openMessageInput: function (title, item) {
                if (item.length > 0) {
                    if (item.length < 200) {
...
<style scoped>
    ...
    .box>>>.el-form--inline .el-form-item__label {
        overflow: hidden;
